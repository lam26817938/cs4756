...
class DriveDataGenerator(image.ImageDataGenerator):
    ...
    def random_transform_with_states(self, x, seed=None):
        ...
        if self.brightness_range != (0, 0):
            ...
            random_bright = np.random.uniform(low=low, high=high)
            # random_bright = np.random.uniform(low = 1.0-self.brightness_range, high=1.0+self.brightness_range)
            
            #TODO: Write this as an apply to push operations into C for performance
            img = cv2.cvtColor(np.array(x), cv2.COLOR_RGB2HSV)
